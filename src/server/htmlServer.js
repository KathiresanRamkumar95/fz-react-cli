...
app.get('/api/html', (req, res) => {
  ...
  var file = fs.readFileSync(
    path.join(appPath, 'components', filePath) + '.html',
    'utf8'
  );
  ...
  if (fileNames.length > 1) {
    cssfile =
      '<style>' +
      fs.readFileSync(
        path.join(appPath, 'components', remainingPath + fileNames[1]) + '.css',
        'utf8'
      ) +
      '</style>';
  } else {
    cssfile =
      '<style>' +
      fs.readFileSync(
        path.join(appPath, 'components', filePath) + '.css',
        'utf8'
      ) +
      '</style>';
  }
  ...
  var styles = Object.keys(customComponent).reduce((res, next) => {
