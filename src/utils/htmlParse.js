  ...
  function lstringify(buff, doc) {
    switch (doc.type) {
      ...
      case 'tag':
        ...
        return (
          buff + doc.children.reduce(lstringify, '') + '</' + doc.name + '>'
        );
      case 'component':
...
exports.parse = function parse(html, options) {
  ...
  html.replace(tagRE, function(tag, index) {
    ...
    if (isOpen) {
      ...
      if (
        current.type === 'tag' &&
        current.name.charCodeAt(0) > 64 &&
        current.name.charCodeAt(0) < 91
      ) {
      ...
      if (current.type == 'component') {
        if (!customComponent[current.name]) {
          ...
          if (a.length > 1) {
            ...
            cssPath =
              cssPath + '/' + current.name + '/' + current.name + '.css';
          }
          customComponent[current.name] = {};
          customComponent[current.name].ast = parse(
            fs
              .readFileSync(options.appPath + path, 'utf8')
              .replace(/\n/g, '')
              .trim()
              .toString(),
            options
          );
          customComponent[current.name].style =
            '<style>' +
            fs.readFileSync(options.appPath + cssPath, 'utf8') +
            '</style>';
        }
      }
      arr[level] = current;
