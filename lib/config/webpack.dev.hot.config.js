...
module.exports = {
  entry: {
    main: [require.resolve('../hmrClient') + ('?hmrPath=' + url), 'babel-polyfill', require.resolve('../wmsClient') + ('?wmsPath=wss://' + host + ':' + port), require.resolve('react-error-overlay'), path.join(appPath, appFolder, mig ? 'migration.js' : 'index.js')]
  },
  ...
  new CaseSensitivePathsPlugin(), new webpack.IgnorePlugin(/^\.\/locale$/, /moment$/), new webpack.optimize.CommonsChunkPlugin({
    name: 'vendor',
    chunks: ['main'],
    minChunks: isVendor
