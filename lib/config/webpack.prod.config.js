...
var plugins = [new CaseSensitivePathsPlugin(), new webpack.IgnorePlugin(/^\.\/locale$/, /moment$/), new webpack.optimize.CommonsChunkPlugin({
  name: 'vendor',
  chunks: ['main'],
  minChunks: isVendor
...
var main;
if (performanceFlag) {
  var triggerAutomation = require('../triggerAutomation');
  plugins.push(new triggerAutomation());
  main = ['babel-polyfill', 'fz-perf-tool', path.join(appPath, appFolder, mig ? 'migration.js' : 'index.js')];
} else {
  main = ['babel-polyfill', path.join(appPath, appFolder, mig ? 'migration.js' : 'index.js')];
}
