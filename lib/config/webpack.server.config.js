...
module.exports = {
  ...
  plugins: [new CaseSensitivePathsPlugin(), new webpack.optimize.ModuleConcatenationPlugin(),
  /*new i18nPlugin({
    appPath: appPath,
    context: context
  }),*/
  ...
  new webpack.DefinePlugin({
    ...
    __SERVER__: true
  }), new WebpackMd5Hash(), new webpack.optimize.UglifyJsPlugin({
    compressor: {
      warnings: false
    }
  })],
  module: {
    rules: [{
      ...
      test: /\.css$/,
      use: ['isomorphic-style-loader', 'css-loader?modules&localIdentName=' + className]
    }, {
      test: /\.jpe?g$|\.gif$|\.png$/,
      use: ['url-loader?limit=1000&name=' + (hash ? './images/[name].[hash].[ext]' : './images/[name].[ext]')]
    }, {
