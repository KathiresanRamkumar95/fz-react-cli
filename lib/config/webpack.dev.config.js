...
module.exports = {
  entry: {
    main: ['babel-polyfill', require.resolve('../wmsClient') + ('?wmsPath=wss://' + host + ':' + port), path.join(appPath, appFolder, mig ? 'migration.js' : 'index.js')]
  },
  devtool: 'eval',
  output: {
  ...
  plugins: [new CaseSensitivePathsPlugin(), new webpack.IgnorePlugin(/^\.\/locale$/, /moment$/), new webpack.optimize.CommonsChunkPlugin({
    name: 'vendor',
    chunks: ['main'],
    minChunks: isVendor
