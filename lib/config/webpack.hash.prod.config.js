...
var isVendor = function isVendor(_ref) {
  ...
  return userRequest && userRequest.indexOf('node_modules') >= 0 && userRequest.indexOf('.css') == -1;
};
...
module.exports = {
  ...
  new webpack.IgnorePlugin(/^\.\/locale$/, /moment$/), new webpack.optimize.CommonsChunkPlugin({
    ...
    __SERVER__: false
  }), new WebpackMd5Hash(), new ChunkManifestPlugin({
  ...
  module: {
    rules: [{
      ...
      use: [{
        ...
        options: {
          limit: 1000,
          name: hash ? './images/[name].[hash].[ext]' : './images/[name].[ext]',
      ...
      use: [{
        ...
        options: {
          limit: 1000,
          name: hash ? './fonts/[name].[hash].[ext]' : './fonts/[name].[ext]',
          publicPath: function publicPath(url) {
