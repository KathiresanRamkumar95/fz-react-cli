...
if (branch) {
	app.post('/repo/merge', function (req, res) {
		...
		if (ref && ref.endsWith(branch)) {
			var results = (0, _child_process.spawnSync)('git', ['pull', 'origin', branch], {
				stdio: 'inherit'
			});
		}
		res.send('done');
...
var referenceMode = function referenceMode() {
	...
	_fzScreenshotTest2.default.run({
		...
		server.close();
		wMid.close();
		var result = (0, _child_process.spawnSync)('cp', ['-r', _path2.default.join(__dirname, '..', '..', 'templates', 'screenShotReport'), _path2.default.join(appPath, 'screenShots')], { stdio: 'inherit' });
