...
var repoUrl = process.env.npm_config_repo_url || 'https://vimalesan.a@git.csez.zohocorpin.com/zohodesk/supportportal.git';
...
var streamObj = createEventStream(5000, function () {
  return { repoBranch: repoBranch, commitHash: commitHash, isStart: serverProcess ? true : false };
});
var serverProcess;
...
app.get('/node/clone', function (req, res) {
  ...
  output = cp.spawnSync('git', ['clone', repoUrl, '-b', repoBranch, repoBranch], {
    encoding: 'utf8'
  });
...
app.get('/node/start', function (req, res) {
  if (!serverProcess) {
    ...
    console.log(test.stdout);
    serverProcess = cp.spawn('npm', ['run', 'serverrender', '--server:port=' + port], {
      encoding: 'utf8',
      shell: true,
      cwd: path.join(appPath, repoBranch, clientAppPath)
    });
...
app.post('/node/deploy', function (req, res) {
  if (serverProcess) {
    ...
    if (serverProcess.kill('SIGTERM')) {
      ...
      serverProcess = cp.spawn('npm', ['run', 'serverrender', '--server:port=' + port, '--', JSON.stringify(deployObj.initialJS)], {
        encoding: 'utf8',
        shell: true,
        cwd: path.join(appPath, repoBranch, clientAppPath)
      });
    ...
    console.log('server build', test.stdout);
    serverProcess = cp.spawn('npm', ['run', 'serverrender', '--server:port=' + port, '--', JSON.stringify(JSON.stringify(deployObj))], {
      encoding: 'utf8',
      shell: true,
      cwd: path.join(appPath, repoBranch, clientAppPath)
    });
