...
app.get('/docs/all.html', function (req, res) {
  res.sendFile(path.join(appPath, 'docs', 'all.html') || path.join(__dirname, '..', '..', 'docs', 'all.html'));
});
...
var server = app.listen(port, function (err) {
  ...
  if (!process.env.npm_config_server_host && !process.env.npm_config_server_port) {
    ...
    console.log('npm start --server:host=vimal-zt58.tsi.zohocorpin.com --server:port=8080 --server:componentPath=./src/components --repo:branch=branch_name');
  }
