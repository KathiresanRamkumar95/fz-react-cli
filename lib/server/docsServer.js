...
app.use('/docs/external', express.static(path.join(appPath, 'docs', 'external')));
app.use('/docs/js', express.static(path.join(__dirname, '..', '..', 'docs', 'js')));
app.use('/docs/css', express.static(path.join(__dirname, '..', '..', 'docs', 'css')));
...
app.use('/author/get', function (req, res) {
  var compName = req.query.componentName || '';
  exec('git blame ' + compName, function (error, stdout, stderr) {
    res.json({ stdout: stdout });
  });
});

app.use('/cli/version', function (req, res) {
  exec('npm ls fz-react-cli', function (error, version, stderr) {
