...
var result = function result(inp) {
	...
	for (var _i = 0; _i < testFilesArr.length; _i++) {
		var curFileObj = testFilesArr[_i];
		var curSourceFile = testFilesArr[_i].sourcePath;
		var coverageData = coverageJson[curSourceFile];
		if (coverageData == undefined) {
			(0, _utils.log)("Can't able to find source for " + testFilesArr[_i].testPath + '\n Please check the file name and the path is correct for test file');
			continue;
		}
