...
var iterateDirectory = function iterateDirectory(srcPath, targetPath, isCopy, extensions) {
    fs.readdirSync(srcPath).forEach(function (fileOrDir) {
        var fromPath = path.join(srcPath, fileOrDir);
        var toPath = path.join(targetPath, fileOrDir);
        if (fs.statSync(fromPath).isDirectory()) {
            if (!fs.existsSync(toPath)) {
                fs.mkdirSync(toPath);
            }
            iterateDirectory(fromPath, toPath, isCopy, extensions);
        } else {
            if (extensions) {
...
var copy = function copy(srcPath, targetPath, isCopy, exts) {
    if (fs.statSync(srcPath).isDirectory()) {
        ...
        var originPath = path.join(targetPath, name);
        if (!fs.existsSync(originPath)) {
            fs.mkdirSync(originPath);
        }
        iterateDirectory(srcPath, originPath, isCopy, exts);
    } else {
...
copy(srcPath, targetPath, isCopy, exts);