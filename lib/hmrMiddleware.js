...
function publishStats(action, statsResult, eventStream, log) {
  ...
  bundles.forEach(function (stats) {
    ...
    if (
    //  !force &&
    stats && (!stats.errors || stats.errors.length === 0) && stats.assets && stats.assets.every(function (asset) {
    ...
    if (stats.errors.length > 0) eventStream.publish({
      type: 'errors',
      data: stats.errors
    });else if (stats.warnings.length > 0) eventStream.publish({
      type: 'warnings',
      data: stats.warnings
    });else eventStream.publish({
      type: 'ok'
    });
  });
}
