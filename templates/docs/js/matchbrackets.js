...
(function(mod) {
  ...
  function findMatchingBracket(cm, where, config) {
    ...
    let dir = match.charAt(1) == '>' ? 1 : -1;
    if (config && config.strict && dir > 0 != (pos == where.ch)) {
      return null; 
    }
    let style = cm.getTokenTypeAt(Pos(where.line, pos + 1));

    let found = scanForBracket(
      cm,
  ...
  function scanForBracket(cm, where, dir, style, config) {
    let maxScanLen = (config && config.maxScanLineLength) || 10000;
    let maxScanLines = (config && config.maxScanLines) || 1000;

    let stack = [];
    let re = config && config.bracketRegex ? config.bracketRegex : /[(){}[\]]/;
    let lineEnd =
      dir > 0
  ...
  function matchBrackets(cm, autoclear, config) {
    ...
    let maxHighlightLen = cm.state.matchBrackets.maxHighlightLineLength || 1000;
    let marks = [],
